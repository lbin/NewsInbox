...
import (
	...
	mgodb "learnerai/models/mgodb/paper"
	milvus "learnerai/models/milvus/paper"
	"math"
	"net/http"
	"strings"
...
func computeEmbedding(ctx *gin.Context, docs []mgodb.PaperWithEmbedding) ([]mgodb.EmbeddingVector, error) {
	n := len(docs)
...
func ComputeEmbeddingFromText(ctx *gin.Context, title, abstract string) ([]float32, error) {
	v, err := computeEmbedding(ctx, []mgodb.PaperWithEmbedding{{Title: title, Abstract: abstract}})
	if err != nil {
...
func SearchByEmbedding(ctx *gin.Context, embedding []float32, topK int) (milvus.ScoredPaperRecords, error) {
	results, err := milvus.SearchPaperRecords(
		ctx,
		milvus.PaperRecords{{Embedding: embedding}}, // single search
		topK,
		"",   // no filter
		0, 0, // no offset or limit
	)
	if err != nil {
		logger.Errorf(ctx, "SearchPaperRecords by embedding failed [err=%+v]", err)
		return nil, err
	}
	if len(results) == 0 {
		err = fmt.Errorf("empty result list")
...
func joinTextForReRank(pp mgodb.PaperWithEmbedding) string {
