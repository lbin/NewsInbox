...
func (r PaperRecord) floatVector() entity.FloatVector {
	return r.Embedding
}

func (r PaperRecord) vector() entity.Vector {
	return r.floatVector()
}

type PaperRecords []PaperRecord

func (s PaperRecords) vectors() []entity.Vector {
...
func GetPaperRecords(ctx *gin.Context, arxivIds []string, start, limit int64) (PaperRecords, error) {
	c := common.GetClient()
...
func DeletePaperRecords(ctx *gin.Context, arxivIds []string) error {
	c := common.GetClient()
	wctx, cancel := common.GetContext(ctx)
	defer cancel()
	...
	if err != nil {
		logger.Errorf(ctx, "delete paper record failed [err=%+v]", err)
		return err
	}
...
type ScoredPaperRecord struct {
	PaperRecord
	Score float32 `json:"score"`
}

type ScoredPaperRecords []ScoredPaperRecord

func fromSearchResult(r client.SearchResult) (ScoredPaperRecords, error) {
	prs, err := fromResultSet(r.Fields)
	if err != nil {
		return nil, err
	}

	sprs := make([]ScoredPaperRecord, r.ResultCount)
	for i := 0; i < r.ResultCount; i++ {
		sprs[i].PaperRecord = prs[i]
...
func SearchPaperRecords(ctx *gin.Context, inps PaperRecords, topK int, filter string, start, limit int64) ([]ScoredPaperRecords, error) {
	c := common.GetClient()
	wctx, cancel := common.GetContext(ctx)
	defer cancel()
...
func QueryPaperRecords(ctx *gin.Context, expr string, start, limit int64) (PaperRecords, error) {
	c := common.GetClient()
	wctx, cancel := common.GetContext(ctx)
	defer cancel()
