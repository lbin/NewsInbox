...
class WechatMessage(ChatMessage):
    def __init__(self, itchat_msg, is_group=False):
        ...
        if itchat_msg["Type"] == TEXT:
            ...
            self.content = TmpDir().path() + itchat_msg["FileName"]  # content直接存临时目录路径
            # self._prepare_fn = lambda: itchat_msg.download(self.content)
            self._prepare_fn = itchat_msg.download(self.content)
            logger.info(self._prepare_fn)
        elif itchat_msg["Type"] == SHARING:
            self.ctype = ContextType.SHARING
            self.content = itchat_msg.get("Url")
            # logger.info("[WX]Sharing message: " + self.content)
        ...
        try:  # 陌生人时候, User字段可能不存在
            ...
            self.my_msg = itchat_msg["ToUserName"] == itchat_msg["User"]["UserName"] and itchat_msg["ToUserName"] != itchat_msg["FromUserName"]
